...
## 2025-11-12T15:43:19-05:00 — Launcher legacy theme variable restoration

- Reintroduced a local `theme` definition in `app/main.py` so downstream launch scripts that still reference `theme.key` stay compatible while the window continues to accept palette keys.
- Ensures RunSpectraApp.cmd and other persisted entry points no longer crash with `NameError: name 'theme' is not defined` after the theming refactor.

## 2025-11-12T15:53:25-05:00 — Launcher theme export for legacy imports

- Promote the resolved theme definition to a module-level export so external launch scripts that import `app.main.theme` regain access to `theme.key`.
- Seed the export with the default palette during import so attribute access remains safe even if the GUI bootstrap fails before resolving user preferences.
