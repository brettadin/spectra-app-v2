...
def test_download_mast_uses_astroquery_helper(
    store: LocalStore, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    service = RemoteDataService(store, session=None)
    payload = tmp_path / "mast-product.fits"
    payload.write_bytes(b"mast")
    ...
    monkeypatch.setattr(service, "_download_via_mast", fake_download)
    ...
    assert called["uri"] == "mast:JWST/product.fits"
    assert result.cached is False
    assert result.cache_entry["source"]["remote"]["provider"] == RemoteDataService.PROVIDER_MAST
    assert Path(result.cache_entry["stored_path"]).exists()
    assert not payload.exists(), "Temporary MAST download should be cleaned up"
